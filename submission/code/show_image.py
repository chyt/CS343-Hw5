...
log = Tkinter.Toplevel(root)
log.geometry('+%d+%d' % (1000,1000))

log_height = 1000
log_width = 1000
log_offset = '0,0'
log_canvas = Tkinter.Canvas(log, offset=canvas_offset, width=log_width, height=log_height, bg='white')
log_canvas.pack()

log_text = classifier.logtext()
log_canvas.create_text(log_width/2, log_height/2, width=log_height, text=str(log_text))
